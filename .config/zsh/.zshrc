...
# 3. Load Nix paths before mise so Nix-provided tools remain fallbacks.
source "$ZRCDIR/nix.zsh"

# 4. Load SDK and development tools configuration
source "$ZRCDIR/sdk.zsh"
source "$ZRCDIR/tools.zsh"

# 5. Load platform-specific base configuration (may override shared settings)
source-safe "$ZRCDIR/$CURRENT_PLATFORM/base.zsh"

# 6. Load shared functions
source "$ZRCDIR/functions.zsh"

# 7. Load platform-specific functions
source-safe "$ZRCDIR/$CURRENT_PLATFORM/functions.zsh"

# 8. Load completions and plugins (before aliases to allow alias completion)
source "$ZRCDIR/plugins.zsh"
source-safe "$ZRCDIR/$CURRENT_PLATFORM/plugins.zsh"

# 9. Load shared aliases
source "$ZRCDIR/alias.zsh"

# 10. Load platform-specific aliases (may override shared aliases)
source-safe "$ZRCDIR/$CURRENT_PLATFORM/alias.zsh"

# 11. Load key bindings last
