...
# =====================================================
# Generate Hyprpaper Configuration
# =====================================================

HYPRPAPER_CONF="$HOME/.config/hypr/hyprpaper.conf"
WALLPAPER_PATH=""

# Try to get current wallpaper path from existing config
if [[ -f "$HYPRPAPER_CONF" ]]; then
    WALLPAPER_PATH=$(grep "^preload" "$HYPRPAPER_CONF" | head -1 | awk -F'= ' '{print $2}')
fi

# Use default if not found
if [[ -z "$WALLPAPER_PATH" ]]; then
    WALLPAPER_PATH="~/.config/hypr/wallpapers/lava-abstract-formation-8k-oc-3440x1440.jpg"
fi

log_info "Generating Hyprpaper configuration..."
...
# Add wallpaper config for each monitor
if [[ "$MODE" == "single" ]]; then
    cat >> "$HYPRPAPER_CONF" << EOF
# Set wallpaper for monitor
wallpaper = ${PRIMARY_MONITOR},${WALLPAPER_PATH}
EOF
else
    cat >> "$HYPRPAPER_CONF" << EOF
...
# Set wallpaper for secondary monitor
wallpaper = ${SECONDARY_MONITOR},${WALLPAPER_PATH}
EOF
fi
...
log_success "Generated: $HYPRPAPER_CONF"
