		...
		config = function()
			...
			local function configure_skkeleton()
				...
				if #dict_paths.available_dicts > 0 then
					...
					vim.notify(
						string.format("🎌 SKK設定完了（%d辞書利用）", #dict_paths.available_dicts),
						vim.log.levels.INFO
					)
				else
					vim.notify(
						"⚠️ SKK辞書が見つかりません。辞書ダウンロードを待機中...",
						vim.log.levels.WARN
					)
				end
			end
			...
			vim.api.nvim_create_user_command("SKKDictClean", function()
				skk_dict.clean()
				vim.notify(
					"🔄 SKK辞書をクリアしました。次回起動時に再ダウンロードされます",
					vim.log.levels.INFO
				)
			end, {
			...
			vim.api.nvim_create_user_command("SKKHealthCheck", function()
				...
				local health_lines = {
					...
					"📦 依存関係:",
					"  plenary.nvim: "
						.. (health.plenary_available and "✅ 利用可能" or "❌ 未インストール"),
					"  curl: " .. (health.curl_available and "✅ 利用可能" or "❌ 未インストール"),
					"",
					"📁 ディレクトリ:",
					"  データディレクトリ書き込み可能: "
						.. (health.data_dir_writable and "✅ OK" or "❌ NG"),
					"",
					"📚 辞書:",
