...
return {
  {
    ...
    ✨ Beautiful • 🚀 Powerful • 🎯 Professional • 💡 Innovative ✨]],
          ...
          keys = {
            ...
            { icon = "󰙅 ", key = "e", desc = "File Explorer", action = ":Neotree" },
            { icon = "󰈔 ", key = "n", desc = "New File", action = ":enew | startinsert" },

            --  Essentials
            { icon = "󰒓 ", key = "c", desc = "Edit Config", action = ":edit ~/.config/nvim/init.lua" },
            { icon = "󰒲 ", key = "l", desc = "Lazy Home", action = ":Lazy" },
            { icon = "󰦛 ", key = "s", desc = "Load Session", action = ":Telescope possession list" },
        ...
        sections = {
          ...
          -- 🌿 Git Status - Simplified & Essential
          {
            ...
            cmd =
            "branch_name=$(git branch --show-current 2>/dev/null) && echo '🌱 '${branch_name:-main} && echo '' && git status --porcelain 2>/dev/null | head -3 || echo '✨ Clean working directory'",
          ...
          -- ⚡ Quick Info - Unified essential info
          {
            ...
            section = "terminal",
            cmd =
            "nvim_version=$(nvim --version 2>/dev/null | head -1 | awk '{print $2}' 2>/dev/null) && plugin_count=$(find ~/.local/share/nvim/lazy -maxdepth 1 -type d 2>/dev/null | wc -l 2>/dev/null || echo '0') && current_time=$(date +'%H:%M' 2>/dev/null) && echo '📝 Neovim '${nvim_version:-'Latest'} && echo '🔌 '${plugin_count}' plugins' && echo '🕐 '${current_time:-'Now'} && echo '✨ Ready to code!'",
      ...
      notifier = {
        enabled = true,
        timeout = 5000,                  -- エラー確認のため5秒に延長
        width = { min = 40, max = 0.5 }, -- 幅を少し広げる
        ...
        -- Keep all notifications in history
        keep = function(notif)
          return true -- すべての通知を履歴に保持
        end,
      },
    ...
    init = function()
      vim.api.nvim_create_autocmd("User", {
        ...
        callback = function()
          ...
          require("snacks").toggle.option("background", { off = "light", on = "dark", name = "Dark Background" }):map(
          "<leader>ub")
    ...
    config = function(_, opts)
      ...
      -- Set vim.ui.select to use Snacks picker (must be after setup)
