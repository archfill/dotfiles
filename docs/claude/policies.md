...
2. Preview Nix changes with `make nix-diff` when Nix files are touched
...
- **Verify changes** with the relevant command before committing (`make nix-diff`, `make nix-rebuild`, or a focused script check)
