...
usage() {
  cat <<'USAGE'
Usage:
  bin/chatgpt-update.sh

The script downloads the latest official ChatGPT Linux .deb packages for
x86_64 and ARM64, reads their shared version, recalculates both Nix hashes,
and updates nix/pkgs/chatgpt/default.nix.

This is intentionally a separate target because the two packages are several
hundred megabytes each and should not be downloaded by the regular nix-update.
USAGE
}
